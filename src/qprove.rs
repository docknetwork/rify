use crate::qreasoner::{Quad, Reasoner};
use crate::qrule::{Entity, LowRule, Rule};
...
fn recall_proof<'a>(
    ...
    if let Some(application) = arguments.remove(to_prove) {
        ...
        for triple in &rules[application.rule_index].if_all {
            recall_proof(
                &[
                    to_global_scope(&application, triple.s.0),
                    to_global_scope(&application, triple.p.0),
                    to_global_scope(&application, triple.o.0),
                    to_global_scope(&application, triple.g.0),
                ]
                .into(),
                arguments,
                rules,
...
mod test {
    ...
    use crate::common::qdecl_rules;
    use crate::qrule::Entity::{Bound as B, Unbound as U};
    use crate::qvalidate::validate;
    use crate::qvalidate::Valid;
